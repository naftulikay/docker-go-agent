...
# install-default-lib
#
# Upon start up of the Docker container, if /var/lib/go-agent is mounted as a volume,
# on first boot it will likely be empty. This is bad, because Go expects certain files
# to live in here. This script creates files and directories in /var/lib/go-agent if
# they are missing from a backup taken at image creation time.

backup_dir="/usr/share/go-agent/default/lib"
target_dir="/var/lib/go-agent"
